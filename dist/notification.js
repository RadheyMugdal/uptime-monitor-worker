...
export class NotificationService {
    static async sendEmail(to, title, message, priority = 'medium') {
        if (!transporter) {
        ...
        const emoji = priorityEmojis[priority] || '⚠️';
    ...
    static async sendWebhook(url, title, message, type, priority = 'medium') {
            ...
            case 'discord':
                ...
                payload = {
                    embeds: [{
                            title: title,
                            description: message,
                            color: colorMap[priority] || colorMap.medium,
    ...
    static async notifyUser(payload) {
            ...
            const promises = channels.map(async (channel) => {
                try {
                    switch (channel.type) {
                        ...
                        case "discord":
                        case "webhook":
                            await this.sendWebhook(channel.value, title, message, channel.type, priority);
                            break;
                        default:
...
export const NotificationHelpers = {
    async sendCriticalAlert(userId, title, message, metadata) {
        return NotificationService.notifyUser({
            userId,
            title,
            message,
    ...
    async sendWarning(userId, title, message, metadata) {
        return NotificationService.notifyUser({
            userId,
            title,
            message,
    ...
    async sendInfo(userId, title, message, metadata) {
        return NotificationService.notifyUser({
            userId,
            title,
            message,
